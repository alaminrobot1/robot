...
def get_exchange_name(exchange_key: str) -> str:
  return EXCHANGES[exchange_key]['name']

def get_exchange_module(exchange_key: str) -> exchange.Exchange:
  module_name = EXCHANGES[exchange_key]['module']
  return import_module(module_name)

def calculate_arbitrage(exchange1: exchange.Exchange, exchange2: exchange.Exchange) -> List[dict]:
  """Calculates the arbitrage opportunities between two exchanges.

  Args:
    exchange1: An Exchange object representing the first exchange.
    exchange2: An Exchange object representing the second exchange.
  ...
  for symbol in common_symbols:
    ...
    if liquidity1 > 0 and deposit_withdraw_network_available1 and liquidity2 > 0 and deposit_withdraw_network_available2:
      ...
      # Exchange trade link generation
      exchange1_symbol_link = symbol.replace('/', '_') if exchange1 in ('exchanges.mexc', 'exchanges.gateio', 'exchanges.binance') else symbol.replace('/', '/')
      exchange2_symbol_link = symbol.replace('/', '_') if exchange2 in ('exchanges.mexc', 'exchanges.gateio', 'exchanges.binance') else symbol.replace('/', '/')

      exchange1_trade_link = f"{exchange1_trade_base_url}{exchange1_symbol_link}"
      exchange2_trade_link = f"{exchange2_trade_base_url}{exchange2_symbol_link}"
